...
export const encryptText = (
    ...
    let { igonereColumns, sign: signed } = options || {
      igonereColumns: [],
      sign: '',
    };
    let signature = '';
    ...
    // To ensure urls does not change when the page is refreshed, we are using a fixed random number.
    const random = 531787374115; //Math.ceil(Math.random() * Date.now());
  ...
  export const decryptText = (
    ...
    const { igonereColumns, sign: signed } = options || {
      igonereColumns: [],
      sign: '',
    };
    let signature = '';
    ...
    for (let i = 0, start = 0; i < keyLength; i++) {
      ...
      start = cursor;
      cursor = diff[char]
        ? start + uniformRomColumned + 1
        : start + uniformRomColumned;
    ...
    let isTrusted =
      inversedText.startsWith(signature) && lengthRegexp.test(inversedText);
    if (isTrusted) {
      let trueLength: any = (
        inversedText.match(/(\?length[1-9][0-9]*)$/gs) as string[]
      )[0].split('length')[1];
      trueLength = Number(trueLength);
      if (!!trueLength) {
        inversedText = inversedText
          .replace(signature, '')
          .replace(lengthRegexp, '');
